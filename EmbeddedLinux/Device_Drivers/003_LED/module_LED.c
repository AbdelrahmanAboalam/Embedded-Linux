...
#define DRIVER_NAME "Aboalam_Driver"
dev_t mydevice_id;
...
ssize_t driver_read(struct file *file, char __user *userr, size_t count, loff_t *loff)
{
...
ssize_t driver_write(struct file *file, const char __user *userr, size_t count, loff_t *loff)
{
...
static int __init lkm_init(void)
{
    ...
    if (NULL == (iti_class = class_create(root,"iti_class_2")))
    {
    ...
    if (cdev_add(&device_driver1, mydevice_id, 1) == -1)
    {
     ...
     if (cdev_add(&device_driver2, mydevice_id, 1) == -1)
    {
        printk("cdev add failed\n");
    }
    ...
    cdev_init(&device_driver2, &file_op);
